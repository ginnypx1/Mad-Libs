...
directions = Label(text='Fill in the blanks!', height=2)
...
# create the form of words
def makeform(root, questions):
	answers = []
...
entries = makeform(root, questions)

but = Button(root, text='Tell me a story!', height=2, command=(lambda: fetch(entries)))
...
"Ahhhh, this {} is just right," {} said happily and ate it all up.
	...
	pg1_btn = Button(story, text='Keep going!', height=2, command=(lambda: story2(answers)))
...
{} had just about enough by now, so {} went upstairs to the bedroom. {} lay down in the first {}, but it was too {}. Then {} lay down in the second {}, but it was too {}. Then {} lay down in the third {} and it was just right. {} fell asleep.
	...
	pg2_btn = Button(story2, text='Tell me the rest!', height=2, command=(lambda: story3(answers)))
...
""".format(answers[0].get(), answers[1].get(), answers[3].get(), answers[1].get(), answers[3].get(), answers[1].get(), answers[3].get(), answers[1].get(), answers[6].get(), answers[1].get(), answers[6].get(), answers[1].get(), answers[6].get(), answers[1].get(), answers[1].get(), answers[9].get(), answers[9].get(), answers[1].get(), answers[9].get(), answers[1].get())
	...
	pg3_btn = Button(story3, text='What is going to happen?', height=2, command=(lambda: story4(answers)))
...
THE END""".format(answers[0].get(), answers[1].get(), answers[0].get(), answers[0].get(), answers[0].get(), answers[1].get(), answers[1].get())
