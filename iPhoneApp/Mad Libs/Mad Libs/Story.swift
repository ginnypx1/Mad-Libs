...
var story: [String] = [
    ...
    "After \(answers[0]) had eaten the three \(answers[1])'s breakfasts, \(answers[0]) was feeling a little tired. So \(answers[0]) walked into the living room and saw 3 \(answers[6])s. \(answers[0]) sat in the first \(answers[6]) to rest \(answers[0])'s feet. \n \"This \(answers[6]) is too \(answers[7])!\" \(answers[0]) exclaimed. \n So \(answers[0]) sat in the second \(answers[6]). \n \"This \(answers[6]) is too \(answers[8])!\" \(answers[0]) whined. \n So \(answers[0]) tried the last and smallest \(answers[6]). \n \"Ahhhh, this \(answers[6]) is just right,\" \(answers[0]) sighed. But just as \(answers[0]) settled down into the \(answers[6]) to rest, it broke into pieces!",
    "\(answers[0]) had just about enough by now, so \(answers[0]) went upstairs to the bedroom. \(answers[0]) lay down in the first \(answers[9]), but it was too \(answers[10]). Then \(answers[0]) lay down in the second \(answers[9]), but it was too \(answers[11]). Then \(answers[0]) lay down in the third \(answers[9]) and it was just right. /n \(answers[0]) fell asleep.",
